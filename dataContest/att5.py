...
# to do tomorrow add features rating length, avg support, weighted average support, last length, support
...
trX=np.ndarray(shape=(len(rtr), 12))
...
id1=[]
ind=0
teX=np.ndarray(shape=(len(rte),12))
a5=np.zeros(len(rte))
for row in rte:
    id1.append(row[0])
    em=row[1]
